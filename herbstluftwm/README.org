...
#+begin_src bash :tangle "~/.config/herbstluftwm/autostart"
  ...
  hc attr theme.title_font 'JetBrainsMono Nerd Font:size=12:style=Regular'
...
#+begin_src bash :tangle "~/.config/herbstluftwm/panel.sh" :tangle-mode (identity #o777)
  ...
  BG="#1C1E26"
  ACCENT="#8f93db"
  FONT="-misc-cozettevector-medium-r-normal--13-0-0-0-p-0-iso10646-1"

  # Pre-define colors for sed to make the tag loop faster
  ...
  get_bar_data() {
      ...
      # 3. Format the remaining elements
      formatted_tags=""
      for tag in "${tags_array[@]}"; do
  	status=${tag:0:1}
  	name=${tag:1}
  	case "$status" in
  	    '#'|'-') formatted_tags+="%{B$ACCENT}%{F$BG} $name %{F-}%{B-}" ;;
  	    ':'|'+') formatted_tags+="%{F$FG} $name %{F-}" ;;
  	    '!')     formatted_tags+="%{F#BF616A} $name %{F-}" ;;
  	    '.')     formatted_tags+="%{F#555555} $name %{F-}" ;;
  	    ,*)       formatted_tags+=" $name " ;;
  	esac
      done
      ...
      cpu_usage=$(( ( (TOTAL - PREV_TOTAL) - (IDLE - PREV_IDLE) ) * 100 / (TOTAL - PREV_TOTAL + 1) ))
      PREV_TOTAL=$TOTAL; PREV_IDLE=$IDLE

      # Meminfo is slightly more efficient than awk for single variables
      ...
      echo "%{l}$formatted_tags %{c} $date_str %{r} %{F$ACCENT}Disk:%{F-} ${disk} %{F$ACCENT}Vol:%{F-} ${vol}%  %{F$ACCENT}Mem:%{F-} $ram  %{F$ACCENT}CPU:%{F-} $cpu_usage%% "
  }
  ...
  # This approach listens to herbstclient but also forces a tick every 5s
  # without creating a massive process pipe.
  (
      while true; do
  	get_bar_data
  	# Wait for a herbstclient event OR timeout after 5 seconds
  	read -t 1 -u 3 2>/dev/null || true
