...
** Sofa
Tested sofa hrir, but havn't found a sofa file i like yet, so disabling
...
#+begin_src conf :tangle no :tgle "~/.config/pipewire/filter-chain.conf.d/spatializer.conf" :noweb yes
  context.modules = [
      { name = libpipewire-module-filter-chain
          args = {
...
#+begin_src conf :tangle no :tgle (if (string= system-name "heimdall") "~/.config/pipewire/filter-chain.conf.d/spatializer.conf" "")
                      node.target    = "alsa_output.usb-Fosi_Fosi_Audio_ZH3-00.analog-stereo"
#+end_src

#+begin_src conf :tangle no :tgle "~/.config/pipewire/filter-chain.conf.d/spatializer.conf"
                      audio.channels = 2
                      audio.position = [ FL FR ]
                      }
                  }
...
#+begin_src systemd :tangle no :tgle "~/.config/systemd/user/filter-chain.service.d/override.conf"
  [Unit]
