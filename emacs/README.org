...
#+begin_src emacs-lisp
  (use-package svelte-ts-mode
    :ensure (:host github :repo "leafOfTree/svelte-ts-mode" :rev "main")
    :mode "\\.svelte\\'"
    :config
    (with-eval-after-load 'svelte-ts-mode
    ;; Force Tree-sitter to handle indentation
...
#+begin_src emacs-lisp
  ...
  (use-package prettier-js
    :ensure t
    :hook ((typescript-ts-mode tsx-ts-mode js-ts-mode) . prettier-js-mode))
#+end_src
...
#+begin_src emacs-lisp
  (use-package cursory
    ...
    :config
    (cursory-set-preset (or (cursory-restore-latest-preset) 'underscroe))
    (cursory-mode 1))
#+end_src
...
#+begin_src emacs-lisp
  (use-package eglot
    ...
    :config
    ;; Set timeouts for slow language servers
  ...
  ;; Configure basedpyright workspace settings SEPARATELY
  (with-eval-after-load 'eglot
    (defun my/eglot-workspace-config (server)
      "Return workspace configuration for SERVER."
      (when (eq (eglot--major-modes server) '(python-mode python-ts-mode))
        `(:basedpyright
          (:analysis
...
#+begin_src emacs-lisp
  (use-package gptel
    ...
    (setq-default gptel-backend my-openrouter-backend
...
#+begin_src emacs-lisp
  (use-package mise
    ...
    ;; Enable all auto-update options
    (setq mise-update-on-eshell-directory-change t))
    
    ;; ;; Force mise to update when changing directories or opening files
    ;; (add-hook 'find-file-hook #'mise-update-buffer)
    ;; (add-hook 'eshell-directory-change-hook #'mise-update-buffer)
    
    ;; ;; Also update after project switch if using project.el or projectile
    ;; (with-eval-after-load 'project
    ;;   (add-hook 'project-switch-hook #'mise-update-buffer))
    ;;)
#+end_src
...
#+begin_src emacs-lisp
  (use-package project
    :ensure nil
    :config
    (add-to-list 'project-vc-extra-root-markers "mise.toml")
...
#+begin_src emacs-lisp
  ...
  (use-package treesit-fold
    :hook (treesit-major-mode-setup . treesit-fold-mode)
...
#+begin_src emacs-lisp :tangle no
  (use-package combobulate
    ...
    :hook ((python-ts-mode . combobulate-mode)
           ...
           (svelte-ts-mode . combobulate-mode))
    :bind (:map combobulate-mode-map
                ("C-M-<up>" . combobulate-navigate-up)
                ("C-M-<down>" . combobulate-navigate-down)
                ("C-M-<left>" . combobulate-navigate-backward)
                ("C-M-<right>" . combobulate-navigate-forward))
    :config
    ;; Explicitly unbind M-up/down to prevent interference with move-text
    (define-key combobulate-mode-map (kbd "M-<up>") nil)
    (define-key combobulate-mode-map (kbd "M-<down>") nil)
    (define-key combobulate-mode-map (kbd "M-n") nil)
    (define-key combobulate-mode-map (kbd "M-p") nil))
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;; (defvar my/themes '(aura doom-flatwhite)
